...
function Supermarket(props) {
  const {supermarket} = useRoute().params
 ...
 useEffect(()=>{
   const getCategories = ()=>{
      getDataByProperty('categories' , 'layout' , 'supermarket' , (data)=>{
        setCategories(data)
      },
 ...
 useEffect(()=>{
   const getProducts = ()=>{
      getDataByProperty('articles' , 'prestataireId' , supermarket.id , (data)=>{
        setProducts(data)
      },
      (error)=>{
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Détails du supermarket */}
      <View style={styles.detailsContainer}>
        <Image
          source={{ uri: supermarket.logo ||  'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRq9tnFBDLNRGWgXgViBnIF2KEsW4Tqh7fbuQ&s' }}
      ...
      <ScrollView horizontal style={styles.platsList} showsHorizontalScrollIndicator={false}>
          {
            products.map((item , index)=>(
              <TouchableOpacity key={index} style={styles.platItem} onPress={()=>{
